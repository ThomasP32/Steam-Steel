...
export class ChallengeService {
    ...
    private readonly states = new Map<GameId, Map<PlayerId, ChallengeState>>();
    server: Server;
    ...
    assignForPlayer(game: GameClassic, player: Player): PublicChallengeView {
        ...
        this.setState(game.id, player.socketId, state);
        const view = this.publicView(game, player.socketId);
    ...
    onPlayerMove(game: GameClassic, player: Player, to: Coordinate[]) {
        const st = this.getState(game.id, player.socketId);
        if (!st) return;
    ...
    onAttack(game: GameClassic, attacker: Player, damageDealt: number) {
        const a = this.getState(game.id, attacker.socketId);
    ...
    onAttackDodged(game: GameClassic, defender: Player) {
        const d = this.getState(game.id, defender.socketId);
    ...
    async cleanupGame(game: GameClassic, endReason: GameEndReason): Promise<void> {
        ...
        if (endReason !== GameEndReason.NoWinner_Termination) {
            for (const [playerId, state] of gameStates.entries()) {
                if (state.completed) {
                    const userId = this.userSocketService.getUserIdBySocket(playerId);
                    if (userId) {
