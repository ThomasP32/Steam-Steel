...
export class ChallengeService {
    ...
    private pickApplicableAndInit(game: GameClassic): ChallengeState {
      ...
      const type = (candidates.length ? candidates : [ChallengeType.VISIT_TILES_25])[
    ...
    private isApplicable(game: GameClassic, t: ChallengeType): boolean {
      switch (t) {
        case ChallengeType.VISIT_TILES_25:  return ((game.mapSize?.x ?? 0) * (game.mapSize?.y ?? 0)) >= 4;
        case ChallengeType.DEAL_5_DAMAGE:   return (game.players?.length ?? 0) >= 2;
        case ChallengeType.ESCAPE_5_ATTACKS:      return true;
        case ChallengeType.OPEN_2_DOORS:    return (game.doorTiles?.filter(d=>!d.isOpened).length ?? 0) >= 2;
        case ChallengeType.COLLECT_2_ITEMS: return (game.items?.length ?? 0) >= 2;
      }
    }
